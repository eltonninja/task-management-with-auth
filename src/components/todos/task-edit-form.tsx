...
export const EditTaskForm = ({
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <Text
        component="h2"
        className="text-2xl font-bold my-4 text-center text-blue-500"
      >
        {data ? "Update a task" : "Create a task"}
      </Text>
      <form
        onSubmit={formik.handleSubmit}
        className="w-full sm:w-3/4 mx-auto mt-8"
      >
        ...
        <div
          className={`flex ${
            router.query.id ? "justify-between" : "justify-end"
          } mt-4`}
        >
          {!!router.query.id && (
            <Button
          ...
          >
            ...
            {((creating && !router.query.id) ||
              (updating && !!router.query.id)) && <Spinner />}
          </Button>
        </div>
      </form>
    </div>
